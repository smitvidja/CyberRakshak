...
export function setReportMode(mode: "anonymous" | "identified") { sessionStorage.setItem(reportModeKey, mode); }
export function getReportMode() { const value = sessionStorage.getItem(reportModeKey); return value === "anonymous" || value === "identified" ? value : null; }
export function setReportCategoryHint(category: string) { sessionStorage.setItem(reportCategoryHintKey, category); }
export function getReportCategoryHint() { return sessionStorage.getItem(reportCategoryHintKey); }
...
export function addComplaintEvidence(draftId: string, evidence: CitizenEvidence) { const evidenceByDraft = readEvidence(); evidenceByDraft[draftId] = [...(evidenceByDraft[draftId] ?? []), evidence]; sessionStorage.setItem(complaintEvidenceKey, JSON.stringify(evidenceByDraft)); }
