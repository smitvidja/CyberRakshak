...
// The actual "Learning Corner" / awareness content, shared by two different
// wrappers: the logged-in Cyber Warrior dashboard (features/cyber-warriors/
// WarriorResources.tsx) and the public /resources page anyone can reach from
// the home page or the "Learning Corner" nav item. This file owns the content
// once, so the two can never drift out of sync with each other - only the
// chrome around it differs (warrior sidebar vs. plain public page).
//
// It was previously ONLY reachable through the warrior route, and every public
// "Learning Corner" link on the site (nav, home page cards, "Explore now",
// "View all updates", each "Read more") pointed there - which meant a citizen
// clicking any of them landed inside the private warrior dashboard shell
// (Dashboard / My Reports / My Application / Leaderboard / Log out), not a
// normal public page. That was the actual bug being fixed here.

// Awareness posters live in public/images/awareness (generated from the source
// art in demo-assets/Resources by backend/scripts/prepare_awareness_assets.py).
// `tall` marks portrait art so the grid gives it a taller frame instead of
...
export function AwarenessResourcesContent() {
