          ...
          <SurfaceCard className="citizen-category-panel" heading={t("categoryTitle")}>
            <fieldset>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {categories.map((category) => <button aria-pressed={form.categoryId === category.id} data-category-code={category.code} className={["citizen-category-option min-h-24 rounded-[var(--radius)] border p-4 text-left transition-colors", form.categoryId === category.id ? "border-[var(--blue)] bg-[var(--blue-soft)]" : "border-[var(--border)] bg-white hover:bg-[#f5f8fb]"].join(" ")} key={category.id} onClick={() => updateField("categoryId", category.id)} type="button"><strong className="block text-sm text-[var(--navy)]">{category.name}</strong>{category.description ? <span className="mt-2 block text-[13.5px] leading-5 text-[var(--muted)]">{category.description}</span> : null}</button>)}
              </div>
